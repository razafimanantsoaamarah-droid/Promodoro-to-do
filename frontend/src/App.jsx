...
const API_URL = import.meta.env.VITE_API_URL || 'http://0.0.0.0:3000/tasks';
...
function App() {
  ...
  // Nouvelle fonction pour toggle une étape
  const handleToggleStep = async (taskId, stepIndex) => {
    const task = tasks.find(t => t.id === taskId);
    if (!task) return;
    
    // S'assurer que les steps sont des objets { text, completed }
    const updatedSteps = task.steps.map((step, idx) => {
      if (idx === stepIndex) {
        // Si le step est une string simple, on le convertit en objet
        if (typeof step === 'string') {
          return { text: step, completed: true };
        }
        // Si c'est déjà un objet, on toggle completed
        return { ...step, completed: !step.completed };
      }
      // Normaliser les autres steps en objets
      if (typeof step === 'string') {
