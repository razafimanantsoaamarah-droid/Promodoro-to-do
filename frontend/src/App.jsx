...
function App() {
  ...
  const { tasks, addTask, updateTask, deleteTask, toggleComplete, toggleStep } =
    useTasks();
  ...
  // Modifie ta fonction de complétude du timer :
  const handleTimerComplete = (mode) => {
    ...
    if ("Notification" in window && Notification.permission === "granted") {
      new Notification("RMY FOCUS", {
        body: `Félicitations ! Votre session de ${label} est terminée.`,
      });
    }
  ...
  return (
    <div
      className={`min-h-screen ${theme.background} ${theme.textColor || "text-white"} font-sans relative overflow-hidden transition-colors duration-700`}
    >
      ...
      />
      <div
        className={`absolute inset-0 bg-gradient-to-br ${theme.gradient} transition-all duration-1000`}
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <header className="mb-12 flex items-center justify-between">
          <div>
            <h1
              className={`text-4xl font-black bg-gradient-to-r from-${theme.accent}-400 to-indigo-400 bg-clip-text text-transparent`}
            >
              RMY FOCUS
            </h1>
          ...
          <div className="flex items-center gap-4">
            {activeTask && (
              <div
                className={`${theme.cardBg} backdrop-blur-sm px-4 py-2 rounded-full border ${theme.border} hidden md:block`}
              >
                <span className="text-sm opacity-70">Focus : </span>
                <span className={`font-medium text-${theme.accent}-400`}>
                  {activeTask.title}
                </span>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1 space-y-8">
            <div className="sticky top-8 space-y-8">
              ...
              <div
                className={`${theme.cardBg} p-1 rounded-2xl border ${theme.border} flex gap-1`}
              >
                {[
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-6">
              ...
              <div
                className={`${theme.cardBg} px-3 py-1 rounded-lg border ${theme.border}`}
              >
