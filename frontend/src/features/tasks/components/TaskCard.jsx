...
const TaskCard = ({ task, onToggleComplete, onDelete, onEdit, onStartFocus, onToggleStep }) => {
  ...
  return (
    <Motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
      className={`relative ${theme.cardBg} backdrop-blur-2xl border ${theme.border} rounded-[2rem] p-6 shadow-xl transition-all duration-500 ${isCompleted ? 'opacity-50' : 'hover:bg-white/10'}`}>
      ...
      <div className="flex items-start gap-4">
        <button 
          ...
          className={`mt-1 transition-all ${isCompleted ? `text-${theme.accent}-400` : 'text-slate-600 hover:text-emerald-500'}`}
        >
          <CheckCircle2 size={26} strokeWidth={isCompleted ? 3 : 2} />
        </button>
        
        <div className="flex-1 min-w-0">
          <h3 className={`text-xl font-bold truncate ${isCompleted ? 'line-through opacity-50' : 'text-white'}`}>{task.title}</h3>
                ...
                return (
                  <li key={idx} className="flex items-center gap-3 group/step">
                    <button onClick={() => onToggleStep(task.id, idx)}
                      ...
                      ${stepCompleted ? `bg-${theme.accent}-500 border-${theme.accent}-500` : `border-white/20 group-hover/step:border-${theme.accent}-500`}`}>
                      {stepCompleted && <Check size={10} className="text-white" strokeWidth={4} />}
                    </button>
                    <span className={`text-xs ${stepCompleted ? 'opacity-50 line-through' : 'text-slate-300'}`}>{stepText}</span>
                  </li>
                );
              })}
            </ul>
          ...
          <footer className="flex items-center gap-4 mt-5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
            <div className="flex items-center gap-1.5">
              <Clock size={12} className={`text-${theme.accent}-500`} />
        ...
        <div className="flex flex-col gap-2">
          ...
          <button 
            ...
            className="p-2.5 rounded-xl text-rose-500 hover:bg-rose-500/10 transition-colors flex items-center justify-center"
          >
            <Trash2 size={18} />
          </button>
        </div>
